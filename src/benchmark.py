#!/usr/bin/env python3
"""
benchmark.py — varredura fatorial (resolução de captura) x (imgsz) x (modelo).

Mede, para cada célula da grade:
  * qualidade  -> Recall, Precision, mAP50, mAP50-95
                  em DOIS pontos de operação:
                    (a) conf=0.001  -> curva PR completa, é o número comparável
                                       com a literatura
                    (b) conf=0.25   -> recall no ponto de operação real do
                                       sistema de alerta. É este que vale para
                                       a dissertação: alerta com conf 0.001
                                       inunda o COPOM de falso positivo.
  * latência   -> forward puro isolado (mediana + p95 sobre N iterações,
                  com warmup e cuda.synchronize), separado do pré e do
                  pós-processamento que o Ultralytics reporta.

Por que separar os dois eixos:
    resolução de captura  = perda de INFORMAÇÃO  -> mexe no recall
    imgsz da inferência   = custo COMPUTACIONAL  -> mexe na latência
  Colapsar os dois num eixo só (o erro mais comum nesse tipo de experimento)
  torna impossível dizer se o recall caiu porque o pixel sumiu ou porque a
  rede rodou menor. A grade 2D responde as duas perguntas de uma vez.

AVISO sobre a RTX 5080:
    a latência medida aqui NÃO é a latência da Orange Pi 5. A RTX serve como
    ORÁCULO DE ACURÁCIA (as curvas de recall transferem) e como medida
    RELATIVA de custo. O número absoluto de latência de edge tem que sair da
    NPU RK3588. Ver o campo 'device' no CSV — sempre reporte junto.

Uso:
    python src/benchmark.py --config configs/experiment.yaml
    python src/benchmark.py --config configs/experiment.yaml --smoke
    python src/benchmark.py --config configs/experiment.yaml \
        --models yolo11n --sizes 640 320
"""
